...
public class DrivingSimulationLab {
    ...
    public static void changeGear() {
        ...
        if (isEngineOn && speed == 0 && (newGear.equals("P") || newGear.equals("R") || newGear.equals("D"))) {
        // Set the gear based on user input
            gear = newGear;
            System.out.println("Gear changed to " + gear);
    } else {
           System.out.println("Cannot change gear. Make sure the engine is on and entering a valid gear.");
        }
    }
    ...
    public static void accelerate() {
        ...
        if (isEngineOn && gear.equals("D")) {
            ...
            System.out.println("Cannot accelerate. Make sure the engine is on and the gear is in drive (D).");
        }
    }
    ...
    public static void brake() {
        ...
        if (isEngineOn) {
            // Add condition to apply the breaks if the speed is greater than 0
            if (speed > 0) {
                // Decrease the speed by 5 units
                speed -= 5;
                // Ensure that the speed does not go below 0
                if (speed < 0) {
                    speed = 0;
                }
                System.out.println("Breaking. Speed is now " + speed + " km/h");
            } else {
                // If the speed is already 0, display error message
                System.out.println("Car is already stopped.");
            }
        } else {
            // If the engine is off, display error message
            System.out.println("Cannot break. The engine is off.");
        }
    }
